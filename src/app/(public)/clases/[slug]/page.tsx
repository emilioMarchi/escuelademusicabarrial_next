...
import { User, Clock, ArrowLeft, Share2, Music, Users, MapPin } from "lucide-react";
  ...
  return (
    <article className="w-full bg-white">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-x-12 gap-y-8 items-start">
          <div className="lg:col-span-7 order-2 lg:order-1">
            <header className="mb-8">
              ...
              <div className="flex flex-wrap gap-y-4 gap-x-8 mb-8 py-4 border-y border-slate-50">
                ...
                {classItem.max_capacity > 0 && (
                  <div className="flex items-center gap-2">
                    <Users size={14} className="text-green-600" />
                    <span className="text-xs font-bold text-slate-800">Cupo: {classItem.max_capacity}</span>
                  </div>
                )}
                {settings?.address && (
                  <div className="flex items-center gap-2">
                    <MapPin size={14} className="text-green-600" />
                    <span className="text-xs font-bold text-slate-800">{settings.address}</span>
                  </div>
                )}
              </div>
            </header>
          ...
          {classItem.image_url && (
            <div className="lg:col-span-5 order-1 lg:order-2 lg:sticky lg:top-8">
              <div className="relative aspect-[4/5] lg:aspect-square overflow-hidden rounded-[2.5rem] shadow-2xl shadow-slate-200">
                <Image src={classItem.image_url} alt={classItem.name} fill className="object-cover" sizes="(max-w-768px) 100vw, 40vw" priority quality={85} />
              </div>
            </div>
          )}
        </div>
      ...
      {otherClassesData.length > 0 && (
        <DynamicSection title="Otras Clases" description="Explora más disciplinas y encuentra la que mejor se adapte a vos." items={otherClassesData} layout="slider" basePath="/clases" />
      )}

      <Contact category="clases" hasForm={true} customTitle={`Inscribite a ${classItem.name}`} />
    </article>
  );
}